...
- **Every acceptance criterion is a test the builder must write.** The drain runs a red-green TDD loop and feeds it exactly these criteria — it does not read this skill, `/implement`, or any external plan; the ticket body IS the whole prompt. So each criterion must be a single, binary, **diff-verifiable** behavior a test can assert: "returns 404 for an unknown order id" not "handles errors gracefully." One behavior per checkbox; if a criterion needs the word "and," split it. Name the seam the test sits at in "What to build" (an existing HTTP route, an exported function) so the builder tests at the right boundary instead of reaching into internals.
- **Size slices to the builder that will actually pick them up.** Check with `~/.local/bin/budget-tier-read`:
  - `green` / `yellow` (or **no output** — the repo hasn't opted into budget routing; never read that as red): the builder is **Claude Sonnet 5**, on the flat-rate Max subscription. Size a slice at roughly what a competent engineer calls "one sitting" — one vertical path through the layers, landing green on its own. Splitting below that is not free: every extra ticket buys another round of context re-establishment, another PR, another review cycle.
  - `red`: the builder is a **fast-tier model** (DeepSeek Flash) with weaker multi-step planning and a shorter effective attention span for sprawling context. Size *well under* "one sitting," and spell the key interface(s) out explicitly rather than leaving them to be discovered.
  <!-- LOCAL DELTA 2026-08-22: upstream assumed a fast-tier builder unconditionally and said "size smaller than for a human or a frontier model / err toward splitting further". Preserve across upstream re-syncs. -->
- **Either way, re-slice on repeated failure.** If a slice keeps failing on retry (the drain climbs model tiers and still can't land it), the slice is wrong — cut it thinner instead of letting the ladder keep escalating.
