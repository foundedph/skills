...
- `pi-reviewing` present → a session may be live. Check whether the daemon is actually alive before proceeding: `ssh wlq@100.91.238.88 'lsof -i :8791'` (see `automation/scripts/CONTROL-PLANE-DEPLOY.md`). If it is up and recently active, say so and stop — the wait is normal, not a failure.
